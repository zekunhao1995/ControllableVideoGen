...
from dataset.data_loader_ucf_reimpl import UCFReader_traj
...
datareader = UCFReader_traj(is_test=False, max_interval=10)  # add min_ntraj=10, max_ntraj=10 for autoencoding (video prediction) evaluation
train_loader = torch.utils.data.DataLoader(
    datareader, batch_size=args['batch_size'], shuffle=True, collate_fn=datareader.collate_fn, worker_init_fn=datareader.worker_init_fn, num_workers=6, pin_memory=True)
...
visual = VisdomShow('ucf_train_humaneval')
...
def train(epoch):
    ...
    for batch_idx, (img_input, warp_input, img_gt, vid_mask) in enumerate(train_loader):
        ...
        if batch_idx%2000 == 0:
            ...
            torch.save(ckpt, './snapshots/ucf/ckpt_e{}_b{}.pth'.format(epoch, batch_idx))
...
restore('./snapshots/ckpt_e2_b58000.pth')
