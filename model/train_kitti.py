...
datareader = KITTIReader_traj(is_test=False, max_interval=10, min_ntraj=1, max_ntraj=5) # change to min_ntraj=10, max_ntraj=10 for autoencoding (video prediction) evaluation
...
visual = VisdomShow('kitti_train_humaneval')
...
def train(epoch):
    ...
    for batch_idx, (img_input, warp_input, img_gt, vid_mask, img_input_2x) in enumerate(train_loader):
        ...
        if batch_idx%2000 == 0:
            ...
            torch.save(ckpt, './snapshots/kitti/ckpt_e{}_b{}.pth'.format(epoch, batch_idx))
