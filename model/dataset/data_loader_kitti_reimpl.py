...
#from commons import trajs2featmap, drawtrajs
...
class KITTIReader_traj:
    ...
    def __init__(self, is_test=True, max_interval=10, min_ntraj=2, max_ntraj=6, is_eval=False):
        ...
        #self.kitti_png_path = dataset_paths['kitti_png']
        self.kitti_bmp_path = dataset_paths['kitti_bmp']
        if self.is_test:
            self.kitti_traj_h5_path = dataset_paths['kitti_traj_h5_test']
        else:
            self.kitti_traj_h5_path = dataset_paths['kitti_traj_h5_train']
        #print(' -- Dataset png path: {}'.format(self.kitti_png_path))
        print(' -- Dataset bmp path: {}'.format(self.kitti_bmp_path))
        print(' -- Trajectory H5 path: {}'.format(self.kitti_traj_h5_path))
        ...
        h5f = h5py.File(self.kitti_traj_h5_path, 'r', libver='latest')
        traj_db = h5f["/KITTITraj/by_clip"]
        ...
        for clip_name in traj_db:
            ...
            trajs = annot[()]
            vid_no = annot.attrs['VidNo']
            cam_name = annot.attrs['CamName']
            annot_t['TrajLen'] = annot_traj_len
            annot_t['StartFrame'] = annot_clip_start
            annot_t['TrajCount'] = num_trajs
            annot_t['Trajs'] = trajs
            annot_t['VidNo'] = vid_no
            annot_t['CamName'] = cam_name
    ...
    def _getitem_pair(self, idx):
        ...
        for frame_no in range(2):
            #img_path = os.path.join(self.kitti_png_path, '{:02d}'.format(vid_no), cam_name, '{:06d}.png'.format(vid_start_frame+interval*frame_no))
            img_path = os.path.join(self.kitti_bmp_path, '{:02d}'.format(vid_no), cam_name, '{:06d}.bmp'.format(vid_start_frame+interval*frame_no))
            frame = cv2.imread(img_path)
            #frame = np.ones([400,1280, 3])
            if frame is None:
            ...
            if frame_no == 0:
                frame2x = frame
                #frame2x = cv2.resize(frame, (845,256), interpolation=cv2.INTER_AREA)  # W H
            frame = cv2.resize(frame, (422,128), interpolation=cv2.INTER_AREA)  # W H      
            frame = frame[:,83:339,:]
        ...
        vid_2x = np.transpose(frame2x, (2,0,1))[(2,1,0),:,:]
        #vid_mask = find_border(vid_seq[1,:,:,:], threshold=10/255)
        vid_mask = np.ones([1,h_target,w_target], dtype=np.float32)  
        ...
        return img_input, warp_input, img_gt, vid_mask, img_input_2x
    ...
    def _getitem_seq(self, idx):
        ...
        for frame_no in range(interval):
            img_path = os.path.join(self.kitti_bmp_path, '{:02d}'.format(vid_no), cam_name, '{:06d}.bmp'.format(vid_start_frame+frame_no))
            frame = cv2.imread(img_path)
            if frame is None:
                print('Frame read failed:\n   {}'.format(img_path))
                return
        ...
        frame2x = np.transpose(frame2x, (2,0,1))[(2,1,0),:,:]
